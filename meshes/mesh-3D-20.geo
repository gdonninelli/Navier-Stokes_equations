...
// --- Mesh Sizes (Realistiche per il 3D) ---
// Per iniziare il 3D, usa parametri molto più grandi.
// 20 nodi sul diametro sono già sufficienti per vedere la fisica di base.
lc_cyl = 0.005;      
lc_global = 0.05;    
...
// --- Mesh Refinement (Box locale invece di distanza globale) ---
// Usare un box di infittimento attorno al cilindro e alla scia
// è molto più efficiente computazionalmente nel 3D.
Field[1] = Box;
Field[1].VIn = lc_cyl;
Field[1].VOut = lc_global;
Field[1].XMin = 0.0; Field[1].XMax = W;
Field[1].YMin = 0.1; Field[1].YMax = 0.3;
Field[1].ZMin = Cyl_z - 0.1; Field[1].ZMax = Cyl_z + 0.6; // Infittisce anche la scia
